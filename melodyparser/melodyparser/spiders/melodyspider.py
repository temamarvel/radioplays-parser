...
class MelodyspiderSpider(scrapy.Spider):
    ...
    def parse(self, response):
        # 1. Находим карточки
        cards = response.css("div.entity-snippet a::attr(href)").getall()
        for card_url in cards:
            full_url = response.urljoin(card_url)
            yield scrapy.Request(full_url, callback=self.parse_detail)

        # 2. Переход на следующую страницу
        next_page = response.css("liscrapy crawl example -o data.json.pagination__item_type_next a::attr(href)").get()
        if next_page:
